...
function mergeSchemas(_a) {
    ...
    if (typeof resolvers === 'function') {
        ...
        finalResolvers = resolvers.reduce(function (left, right) {
            return utils_1.mergeDeep(left, typeof right === 'function' ? right(mergeInfo) : right);
        }, {});
        if (Array.isArray(resolvers)) {
            finalResolvers = resolvers.reduce(utils_1.mergeDeep, {});
        }
    }
    else {
    ...
    extensions.forEach(function (extension) {
        mergedSchema = polyfills_1.extendSchema(mergedSchema, extension, {
...
function merge(typeName, candidates) {
    ...
    if (graphql_1.isObjectType(initialCandidateType)) {
        return new graphql_1.GraphQLObjectType({
            name: typeName,
            fields: candidates.reduce(function (acc, candidate) { return (__assign(__assign({}, acc), polyfills_1.toConfig(candidate.type).fields)); }, {}),
            interfaces: candidates.reduce(function (acc, candidate) {
                var interfaces = polyfills_1.toConfig(candidate.type).interfaces;
        ...
        return new graphql_1.GraphQLUnionType({
            name: typeName,
            types: candidates.reduce(function (acc, candidate) { return acc.concat(polyfills_1.toConfig(candidate.type).types); }, []),
        });
    }
    else if (graphql_1.isEnumType(initialCandidateType)) {
        return new graphql_1.GraphQLEnumType({
            name: typeName,
            values: candidates.reduce(function (acc, candidate) { return (__assign(__assign({}, acc), polyfills_1.toConfig(candidate.type).values)); }, {}),
        });
    }
